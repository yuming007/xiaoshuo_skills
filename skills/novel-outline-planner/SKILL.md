...
该工程除 `chapters/` 与 `outlines/` 外，还默认支持：

- `tracking/foreshadows.md`
- `tracking/timeline.md`
- `tracking/character-status.md`

如果这些 tracking 文件存在，默认应读取；如果不存在，不阻塞补纲，但要退回正文与状态文件自行推断。
...
9. `../novel-chapter-writer/references/tracking-protocol.md`
10. `references/tracking-usage.md`
11. `../novel-chapter-writer/references/history-hook-rules.md`
12. 如果工程内存在 `tracking/foreshadows.md`、`tracking/timeline.md`、`tracking/character-status.md`，按当前章相关性抽读
13. 先整理一份本章最简记忆包，只保留与本章冲突和承接直接相关的信息
14. 按任务性质补读共享专项模块：
...
- 钩子优先落在责任链、物证、召见、半句未尽和关系变温上，不要写空泛预告句
...
如果 tracking 文件存在，要明确检查：

- `tracking/foreshadows.md` 里哪些条目会在当前多章内推进
- `tracking/timeline.md` 是否允许把这些差事连续排进来
- `tracking/character-status.md` 是否支持人物关系继续这么走
...
- 章末钩子是否具体到物证、召见、责任链、半句未尽，而非空泛预告
- 如果一次规划多章，这几章是否构成一个清晰的阶段推进，而不是平铺并列
- 是否已经按共享模块规则检查过：
  - 本章目标、阻力、变化、章末拉力
  - 动机、触发、后果是否成立
  - 章末是否能被下一章第一拍接住
- 若 tracking 文件存在，是否检查了伏笔、时间线、角色状态与新章纲是否相容
...
- `../novel-chapter-writer/references/tracking-protocol.md`
- `references/tracking-usage.md`
- `../novel-chapter-writer/references/history-hook-rules.md`
